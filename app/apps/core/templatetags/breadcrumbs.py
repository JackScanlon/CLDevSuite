...
def generate_breadcrumbs(crumbs, header=False):
  ...
  for i, crumb in enumerate(crumbs):
    url = crumb['url'] if i == len(crumbs) - 1 else reverse(crumb['url'])
    title = _(crumb['title'])
...
def breadcrumbs(parser, token):
  ...
  try:
    ...
    raise TemplateSyntaxError("Woops")
...
class BreadcrumbsNode(template.Node):
  ...
  def map_resolver(self, path, rqst):
    crumbs = []
    for i, crumb in enumerate(path):
      if crumb != '' or self.params['includeHome']:
        url = get_url(crumb)
        if url:
          root = ''
          ...
          crumbs.append({'url': root + url.name, 'title': 'Home' if crumb == '' else crumb.capitalize() })
  ...
  def render(self, context):
    ...
    path = rqst.get_full_path().split('/')[:-1]
